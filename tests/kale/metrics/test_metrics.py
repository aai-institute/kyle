...
from kale.metrics import BaseCalibrationError, ECE, MCE, ACE
...
@pytest.fixture(scope="module")
def base_criterion():
    base_criterion = BaseCalibrationError()
...
def test_confidences_sum_to_one(base_criterion):
    ground_truth, confidences = sample_confs_from_dirichlet(n_classes=2, n_samples=1000)
    confidences += 0.5
    with pytest.raises(ValueError):
        assert base_criterion.compute(confidences, ground_truth)
